 ...
 */
@Path("/user")
public class MongoDbExample extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		MongoClientURI uri = new MongoClientURI("mongodb://latha:latha@ds051334.mlab.com:51334/latha");
		MongoClient client = new MongoClient(uri);

		DB db = client.getDB(uri.getDatabase());
		DBCollection users = db.getCollection("sample");
		BasicDBObject query = new BasicDBObject().append("name","deepika");

		BasicDBObject newDocument = new BasicDBObject();
		newDocument.put("name","lakshmi");
			

		users.update(query,newDocument);

		query.put("name","latha");// request.getParameter("name"));
		query.put("password","muddu"); //request.getParameter("password"));
